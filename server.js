...
var config = getValuesFromConfigJson(jsonPath);//jsonPath 구조체에서 configJson파일의 정보를 config객체에담음
config = getBashParameters(config, BASH_COLORS_HELPER);//bash color도 받음

// if user didn't modifed "PORT" object
// then read value from "config.json"
if(PORT === 9001) {//port 정보를 수정하지 않았다면 configJson 파일의 port 번호를 받음
    PORT = config.port;
}
if(isUseHTTPs === false) {//HTTPs를 쓰는 것이 false라면 그것을 받아옴
...
function serverHandler(request, response) {
    ...
    try {
        ...
        try {//config.json의 dirpath가 입력이 안되있다면 객체에 null값 입력
            if (!config.dirPath || !config.dirPath.length) {
                config.dirPath = null;
            }

            uri = url.parse(request.url).pathname;//url주소에서 pathname 받아와서 변수에 저장
        ...
        //해당 demo를 선택했을시 실행될 html을 demos폴더에서 실행
...
httpApp = httpApp.listen(process.env.PORT || PORT, process.env.IP || "192.168.43.98", function() {
